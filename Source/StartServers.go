...
var CodeMazeHostMap map[string]*MazeHost
...
func listen(conn *websocket.Conn) {
	for {
		...
		if len(ClientMessage) == 14 && ClientMessage[len(ClientMessage)-9:] == "JoinGame\n" {
			...
			webapp, err := CreateWebApp(conn, messageType, mh)
			if err != nil {
...
func StartSocketServer() {
	...
	CodeMazeHostMap = make(map[string]*MazeHost)
...
func handleConnection(conn net.Conn) {
	...
	if len(ClientMessage) == 14 && ClientMessage[:9] == "StartGame" {
		...
		CodeMazeHostMap[Code] = CreateHost(conn, reader, Code)
		return
	}
